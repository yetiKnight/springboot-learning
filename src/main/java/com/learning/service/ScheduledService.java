...
public class ScheduledService {
    ...
    public void fixedDelayTask() {
        log.debug("固定延迟任务执行，时间：{}", LocalDateTime.now().format(formatter));
        try {
            // 模拟任务执行
            Thread.sleep(1000);
    ...
    public void fixedRateTask() {
        log.debug("固定频率任务执行，时间：{}", LocalDateTime.now().format(formatter));
        try {
            // 模拟任务执行
            Thread.sleep(500);
        } catch (InterruptedException e) {
    ...
    /**
     * 初始延迟执行 - 延迟30秒后开始，每2分钟执行一次
     * 面试重点：initialDelay的使用
     */
    @Scheduled(initialDelay = 30000, fixedRate = 120000)
    @LogExecutionTime
    public void initialDelayTask() {
        log.debug("初始延迟任务执行，时间：{}", LocalDateTime.now().format(formatter));
        try {
            // 模拟任务执行
            Thread.sleep(300);
        } catch (InterruptedException e) {
    ...
    public void cronTask() {
        log.debug("Cron任务执行，时间：{}", LocalDateTime.now().format(formatter));
        try {
            // 模拟任务执行
            Thread.sleep(200);
        } catch (InterruptedException e) {
    ...
    public void exceptionTask() {
        log.debug("异常处理任务执行，时间：{}", LocalDateTime.now().format(formatter));
        try {
            // 模拟随机异常
            if (Math.random() > 0.7) {
                throw new RuntimeException("模拟定时任务异常");
            }
            log.debug("异常处理任务执行成功");
        } catch (Exception e) {
    ...
    public void longRunningTask() {
        log.debug("长时间运行任务开始，时间：{}", LocalDateTime.now().format(formatter));
        try {
            // 模拟长时间运行的任务
            Thread.sleep(5000);
            log.debug("长时间运行任务完成，时间：{}", LocalDateTime.now().format(formatter));
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    public void conditionalTask() {
        log.debug("条件执行任务检查，时间：{}", LocalDateTime.now().format(formatter));
