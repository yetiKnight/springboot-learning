...
```mermaid
graph TD
    A[main方法<br/>应用入口点，启动SpringBoot应用] --> B[创建SpringApplication实例<br/>初始化SpringBoot应用配置]
    B --> C[运行SpringApplication.run<br/>执行完整的启动流程]
    C --> D[准备环境<br/>加载配置文件，设置属性源]
    D --> E[创建应用上下文<br/>根据应用类型创建对应的ApplicationContext]
    E --> F[准备上下文<br/>配置上下文环境，加载Bean定义]
    F --> G[刷新应用上下文<br/>核心步骤：创建Bean，启动服务]
    G --> H[启动内嵌Web服务器<br/>创建并启动Tomcat等Web服务器]
    H --> I[启动完成<br/>应用完全启动，可以接收请求]
    
    B --> B1[推断Web应用类型<br/>判断是SERVLET/REACTIVE/NONE类型<br/>根据classpath中的类进行推断]
    B --> B2[加载ApplicationContextInitializer<br/>从spring.factories加载上下文初始化器<br/>在上下文刷新前执行初始化逻辑]
    B --> B3[加载ApplicationListener<br/>从spring.factories加载应用事件监听器<br/>监听应用启动过程中的各种事件]
    
    F --> F1[设置环境<br/>将准备好的环境设置到应用上下文中<br/>包括配置文件、系统属性等]
    F --> F2[应用初始化器<br/>执行所有ApplicationContextInitializer<br/>在上下文刷新前进行初始化]
    F --> F3[加载主应用类的Bean定义<br/>注册主应用类为Bean定义<br/>包含@SpringBootApplication等注解]
    F --> F4[通知监听器<br/>通知监听器上下文已准备和加载<br/>发布ContextPrepared和ContextLoaded事件]
    
    G --> G1[准备刷新<br/>设置启动时间，初始化属性源<br/>验证必需属性，存储早期监听器]
    G --> G2[获取BeanFactory<br/>创建或获取Bean工厂实例<br/>加载Bean定义，设置序列化ID]
    G --> G3[准备BeanFactory<br/>配置Bean工厂的基础设施<br/>设置类加载器、表达式解析器等]
    G --> G4[后处理BeanFactory<br/>子类可重写，添加自定义处理<br/>SpringBoot中通常为空实现]
    G --> G5[调用BeanFactoryPostProcessor<br/>执行Bean工厂后处理器<br/>核心：组件扫描和自动配置]
    G5 --> G5A[组件扫描<br/>扫描@Component、@Service等注解的类<br/>注册为Bean定义]
    G5 --> G5B[自动配置<br/>加载spring.factories中的自动配置类<br/>根据条件注解筛选配置]
    G5 --> G5C[条件注解评估<br/>评估@ConditionalOnXxx条件注解<br/>决定是否加载配置类]
    G --> G6[注册BeanPostProcessor<br/>注册Bean后处理器，用于AOP等<br/>在Bean创建前后进行处理]
    G --> G7[初始化MessageSource<br/>初始化国际化消息源<br/>支持多语言消息处理]
    G --> G8[初始化ApplicationEventMulticaster<br/>初始化应用事件广播器<br/>用于发布和监听应用事件]
    G --> G9[启动内嵌服务器<br/>创建并启动Tomcat等Web服务器<br/>在onRefresh()方法中执行]
    G --> G10[注册监听器<br/>注册应用事件监听器<br/>包括静态监听器和Bean监听器]
    G --> G11[实例化单例Bean<br/>创建所有单例Bean，执行依赖注入<br/>触发Bean生命周期回调]
    G --> G12[完成刷新<br/>发布ContextRefreshedEvent事件<br/>启动生命周期处理器]
```
